 ...
 *
 */
define('ywj/simplefileuploader', function(){
	if(!window.Worker){
	...
	var su = function(input, config){
		...
		this.config = $.extend({
			...
			CLS_UPLOAD_FILE_THUMB: 'file-thumb',
			CLS_FILE_DELETE: 'delete-link',
		...
		PRI.button = $('<label class="'+this.config.CLS_UPLOAD_BUTTON+'" for="'+this.id+'">上传文件</label>');
		PRI.file_container = $('<span class="'+this.config.CLS_UPLOAD_FILE_THUMB+'"><a href="" target="_blank"></a><span class="'+this.config.CLS_FILE_DELETE+'">删除文件</span></span>');
		PRI.file_link = $('a', PRI.file_container);

		var delete_btn = $('span.'+this.config.CLS_FILE_DELETE, PRI.file_container);
		...
		PRI.file_container.appendTo(PRI.container);
		...
		if(PRI.input.val()){
			...
			var name = srcArr[srcArr.length-1];
			this.onSuccess(null, {src:src, value:val, name:name});
		} else {
		...
		file_input.on('change', function(){
			...
			if (file_type) {
				if(upload_url.indexOf("&")>-1){
					upload_url += '&file_type='+file_type;
				}else{
					upload_url += '?file_type='+file_type;
				}
			}
	...
	su.prototype.updateDomState = function(to, message){
		...
		switch(to){
			...
			case this.config.CLS_UPLOAD_NORMAL:
				PRIVATES[this.id].button.html('上传文件');
				break;
	...
	su.prototype.onSuccess = function(message, data){
		...
		PRIVATES[this.id].file_link.attr('href', data.value).text(data.name);
