...
define('www/quote', function(require){
	...
	window.__tooltip_description__ = function (description, is_feature){
		if (is_feature) {
			return '<span class="feature-ico" title="' + description.replace(/"/,'\\"') + '" rel="tooltip">特色</span>';
		}else{
			return '<span class="quote-help" title="' + description.replace(/"/,'\\"') + '" rel="tooltip">?</span>';
		}
	};
	...
	var updateSkuVersionUpdateInfo = function(){
		var $sel = $(this);
		$sel.parents(".craft-sku-row").removeClass("sku-delete-flag");
	};
	 ...
	 * @param $tr
	 */
	var replace_description = function(sku_code, $tr){
		if (SKU_MAP_SUMMARY[""+sku_code+""] && SKU_MAP_SUMMARY[""+sku_code+""]["description"]) {
			var desc = __tooltip_description__(SKU_MAP_SUMMARY[""+sku_code+""]["description"], SKU_MAP_SUMMARY[""+sku_code+""]["is_feature"]);
	...
	var update_sum = function(){
		...
		$('.craft-sku-row *[data-mode=price]', $QUOTE).each(function(){
			...
			if(sku_sel.size()){
				...
				var sku_code = $op.val();
				replace_description(sku_code, $op.parents(".craft-sku-row"));
	 ...
	 * @param node
	 */
	var insert_product = function(product, sku_code, node){
	 ...
	 * @param node
	 */
	var insert_house_item = function(product, sku_code, node){
		var tbl = node.closest('table');
		var $tbody = $('tbody', tbl);
		var unit_idx = $('input[rel=category_unit_idx]', tbl).val();
	 ...
	 * @param node
	 */
	var insert_demo_item = function(product, sku_code, node){
		var tbl = node.closest('table');
		var $tbody = $('tbody', tbl);
		var unit_idx = $('input[rel=category_unit_idx]', tbl).val();
	...
	var update_water_ele_quantity = function(onFinish){
		...
		get_water_ele_base_quantity_list(layout_id, area, function(sku_list){
			...
			$sku_list.each(function(){
				var sku_code = $.trim($(this).val());

				//如果为默认sku
				if(ORIG_HOUSE_SKU_LIST[sku_code]===undefined){
					var found = false;
				...
				if(!found){
					...
					$('[rel=category_id]', $LIST_CON).each(function(){
						if(this.value == sku.category_id){
							insert_house_item(sku, sku.sku_code, $(this));
						}
					});
					has_new_sku = true;
				}
			...
			$sku_list.each(function(){
				var sku_code = $(this).val();
				if(sku_list[sku_code]){
					var $row = $(this).closest('tr');
					$('[data-mode=number]', $row).val(sku_list[sku_code].base_quantity);
					match = true;
				}
			});
			...
			$('[rel=water-ele-point-id]', $QUOTE).each(function(){
				...
				for(var sku_code in sku_map){
					var ext_quantity = sku_map[sku_code];
					if(ext_quantity){
						$sku_list.each(function(){
							var $sel = $(this);
	...
	$QUOTE.delegate('.add-room-product-btn', 'click', function(ev){
		...
		p.listen('onSuccess', function(product_list, sku_code_list){
			for (var k in sku_code_list) {
				var t_s = sku_code_list[""+k+""];
				var product = $.extend(product_list[t_s.product_id], t_s);
				insert_product(product, t_s.sku_code, $this);
			}
	...
	$QUOTE.delegate('.add-demo-category-item-btn', 'click', function(ev){
		...
		p.listen('onSuccess', function(product_list, sku_code_list){
			for (var k in sku_code_list) {
				var t_s = sku_code_list[""+k+""];
				var product = $.extend(product_list[t_s.product_id], t_s);
				insert_demo_item(product, t_s.sku_code, $this);
			}
		});
		p.show();
		return false;
	...
	//sku 改变更新版本更新相关
	$QUOTE.delegate('select[data-mode=sku-list]', 'change', updateSkuVersionUpdateInfo);
	...
	$('.quote-smt-btn', $QUOTE).click(function(){
		var _this = $(this);
		if ($(".sku-delete-flag").length > 0 && !_this.data('no_check_sku')) {
			Msg.show('报价中存在已删除的SKU', 'err');
			return false;
		}
		if(_this.data('flag')){
